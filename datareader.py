...
class PoemData(Dataset):
    ...
    def __getitem__(self, idx):
        line = self.poems[idx]
        token_line = self.tokenizer.encode(line, max_length=self.max_len + 1, truncation=True)
        token, token_type = token_line['input_ids'], token_line['token_type_ids']

        # 修改：强制截断，防止超过 BERT 最大长度限制
        token = token[:self.max_len + 1]
        token_type = token_type[:self.max_len + 1]
